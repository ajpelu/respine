...
shinyServer(
  function(input, output, session){
    ...
    vb <- reactiveValues(doPlot = FALSE)

    observeEvent(input$doPaisaje, {
      # 0 will be coerced to FALSE
      # 1+ will be coerced to TRUE
      vb$doPlot <- input$doPaisaje
      })
    ...
    rasterRich <- reactive({
      ...
      mapa_riqueza <- initRichness(r = rasterIni(), draster = dist_raster,
                                   r_range = myr_range, treedensity = den_pp(),
                                   pastUse = pastUse, rescale = FALSE)
    })

    rasterDisp <- reactive({
      v <- disper(x = rasterIni(), xr = rasterRich(), nf_value = 2, pp_value = 1)
      })


    output$initial_map <- renderPlot({

      if (vb$doPlot == FALSE) return()
    ...
    output$richness_disperTime <- renderPlot({
      ...
      # pineLimit <- rasterToPolygons(rasterIni(), fun=function(x){x==1}, dissolve = TRUE)
      mytheme <- rasterTheme(region = brewer.pal(9, "YlGn"))
